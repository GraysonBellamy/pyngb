        ...
        with tempfile.NamedTemporaryFile(suffix=".ngb-ss3", delete=False) as temp_file:
            with zipfile.ZipFile(temp_file.name, 'w') as z:
                ...
                metadata_fields = [
                    ...
                    (b"\x30\x75", b"\x9e\x0c", b"\x33\x33\x33\x33\x33\x33\x2f\x40"),  # 15.6
                    # Operator
                ...
                for category, field, value in metadata_fields:
                    ...
                    part = (
                        category + b"pad" * 5 + field + b"pad" * 3 +
                        markers.TYPE_PREFIX + data_type + markers.TYPE_SEPARATOR +
                        value + markers.END_FIELD
                    )
                ...
                time_table = (
                    time_header + markers.TABLE_SEPARATOR +
                    b"\x8d\x75" + markers.START_DATA + b"\x05" +
                    b"".join(time_data) + markers.END_DATA
                )
                
                # Temperature column  
                temp_header = b"\x8e\x17" + b"pad" * 10
                temp_table = (
                    temp_header + markers.TABLE_SEPARATOR +
                    b"\x8e\x75" + markers.START_DATA + b"\x05" +
                    b"".join(temp_data) + markers.END_DATA
                )
                ...
                dsc_table = (
                    dsc_header + b"\x9c\x75" + markers.START_DATA + b"\x05" +
                    b"".join(dsc_data) + markers.END_DATA
                )
    ...
    def create_large_mock_data(self, num_points=1000):
        ...
        with tempfile.NamedTemporaryFile(suffix=".ngb-ss3", delete=False) as temp_file:
            with zipfile.ZipFile(temp_file.name, 'w') as z:
                ...
                # Minimal stream 1
                stream1_data = (
                    b"\x75\x17" + b"pad" * 5 + b"\x59\x10" + b"pad" * 3 +
                    markers.TYPE_PREFIX + b"\x1f" + markers.TYPE_SEPARATOR +
                    b"\x10\x00\x00\x00Test Instrument\x00" + markers.END_FIELD
                )
                z.writestr("Streams/stream_1.table", stream1_data)
                ...
                time_table = (
                    b"\x8d\x17" + b"pad" * 5 + markers.TABLE_SEPARATOR +
                    b"\x8d\x75" + markers.START_DATA + b"\x05" +
                    b"".join(float_data) + markers.END_DATA
                )
